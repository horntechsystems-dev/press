...
class Agent:
	...
	def __init__(self, server, server_type="Server"):
		...
		self.port = 443 if self.server not in self.__servers_using_alt_ports else 25052
	...
	def _get_request_url(self, path):
		if self.server_type in ("Server", "Database Server"):
			...
			if proxy:
				proxy_port = 443 if proxy not in self.__servers_using_alt_ports else 25052
				return f"http://{proxy}:{proxy_port}/{self.server}:{self.port}/agent/{path}" if proxy_port == 25052 else f"https://{proxy}:{proxy_port}/{self.server}:{self.port}/agent/{path}"

		server_ip = frappe.db.get_value(self.server_type, self.server, "ip") or self.server
		host = server_ip if self.port == 25052 else self.server
		return f"http://{host}:{self.port}/{path}" if self.port == 25052 else f"https://{self.server}:{self.port}/agent/{path}"
